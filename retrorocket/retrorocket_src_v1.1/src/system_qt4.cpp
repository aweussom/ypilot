...
Network *openNetwork (bool isMaster, char *fieldFilename, int min, int max,
  ...
  if (bsdsocket->init (isMaster, fieldFilename, min, max, aiPlayers)) {
    if (bsdsocket->numberOfShips - bsdsocket->aiPlayers == 1) {
      ...
      nn->numberOfShips = bsdsocket->numberOfShips;
...
BSDSocket::~BSDSocket() {
  if (slaveSocketsRead && slaveSocketsWrite) {
    if (master) {
      for (int i = 0; i < numberOfShips-aiPlayers-1; i++) {
...
bool BSDSocket::init (bool isMaster, char *fieldFilename, int min, int max,
                      int aiPlayers) {
    ...
    for (int i = 1; i < numberOfShips-aiPlayers; i++) {
      if (read (slaveSocketsRead[i-1], &all, sizeof (int)) > 0) {
        ...
        if (all == -1) {
          ...
          for (int j = 1; j < numberOfShips-aiPlayers; j++) {
            if (j != i) {
...
RetrorocketWindow::RetrorocketWindow (QWidget *parent) : QMainWindow (parent) {
...
void setTextColor (int c) {
  ...
  switch (c) {
    ...
    default:
      ...
      retrorocket->config->findID ("color_player", &configContext);
      for (int i = 0; i < (c - PREDEFINED_TEXTCOLORS); i++) {
        retrorocket->config->next (&configContext);
      }
      char color[8];
      snprintf (color, 8, "#%s",
                retrorocket->config->getString (&configContext));
      color[7] = 0;
